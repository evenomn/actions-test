...
5. **完整清单存档**:当天所有符合条件(含未入选)的漏洞都写入 `data/digest-日期.md` 随仓库提交,钉钉只看头部,想看全去仓库翻存档

去重说明:已推送的 CVE 记录在 `data/state.json`(保留 30 天)。注意**已见过的不会重推**——手动 `--no-dedup` 触发不受此限制。

本地调试等效: `python3 monitor.py --dry-run --no-dedup`。
...
Actions → CVE Daily Monitor → Run workflow 手动触发。**勾选 `ignore_dedup`(默认勾上)会直接展示当前窗口的 top 列表且不消耗去重状态**,适合随时手动看;不勾选则完全模拟定时任务(只推新增)。

去重说明:定时任务每天 9 点只推**新增**的高质量漏洞。如果你刚手动跑过一次,当天定时任务里大部分条目已被标记,日报会显示「无新增」并注明去重压制了多少条——这是正常行为,完整清单每天都会存档到 `data/digest-日期.md`。
